...
fn collect_type_args_from_call(callee: &TypedExpr, subst: &Subst) -> Vec<SemTy> {
    // In the typed AST we don't explicitly store which type args were passed
    // at a call site (that's what the mono pass figures out from the return type).
    // For now, if the callee type is Generic we extract its args.
    match &callee.ty {
