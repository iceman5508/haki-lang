/* haki_typeinfer_compiled.c — functions only (runtime stripped) */
/* ── Functions ── */
...
Scope* scopeNew(void) {
    return ({ Scope* __c_Scope = (Scope*)malloc(sizeof(Scope)); __c_Scope->names = haki_array_new(sizeof(void*)); __c_Scope->types = haki_array_new(sizeof(void*)); __c_Scope; });
}

void scopeSet(Scope* sc, const char* name, const char* ty) {
    int64_t i = ((int64_t)0LL);
...
const char* scopeGet(Scope* sc, const char* name) {
    int64_t i = ((int64_t)0LL);
    while ((i < haki_array_length(sc->names))) {
        if ((strcmp((*(const char**)haki_array_get(sc->names, i)), name) == 0)) {
...
int8_t isIntTy(const char* ty) {
    return (strcmp(ty, "int") == 0);
}

int8_t isFloatTy(const char* ty) {
    return (strcmp(ty, "float") == 0);
}

int8_t isStringTy(const char* ty) {
    return (strcmp(ty, "string") == 0);
}

int8_t isBoolTy(const char* ty) {
    return (strcmp(ty, "bool") == 0);
}

int8_t isNumericTy(const char* ty) {
    if ((strcmp(ty, "int") == 0)) {
        return 1;
    }
...
const char* binaryOpC(const char* op, const char* ty) {
    if (((strcmp(op, "+") == 0) && isNumericTy(ty))) {
        return "+";
    }
    if (((strcmp(op, "-") == 0) && isNumericTy(ty))) {
        return "-";
...
const char* builtinReturnTy(const char* name) {
...
const char* inferExprTy(compiler__Expr* e, Scope* sc, typeck__SymTable* sym) {
    ...
    if ((strcmp(tag, "unary") == 0)) {
        ...
        if ((strcmp(op, "-") == 0)) {
            const char* ity = inferExprTy(inner, sc, sym);
            return ity;
        }
        return "void*";
    }
    if ((strcmp(tag, "binary") == 0)) {
        ...
        const char* lty = inferExprTy(l, sc, sym);
    ...
    if ((strcmp(tag, "call") == 0)) {
        const char* name = ({ const char* __match_result;  void* __msc = (void*)e;  int64_t __mtag = ((int64_t*)__msc)[0];  void* __mpayload = ((void**)__msc)[1];  if (__mtag == 7LL) { const char* n = *(const char**)((void**)__mpayload)[0]; void* args = *(void**)((void**)__mpayload)[1]; __match_result = n; } else { __match_result = ""; } __match_result; });
        const char* bty = builtinReturnTy(name);
    ...
    if ((strcmp(tag, "if") == 0)) {
        ...
        if ((haki_array_length(th) > ((int64_t)0LL))) {
            ...
            const char* lastTy = inferStmtYieldTy(last, sc, sym);
    ...
    if ((strcmp(tag, "match") == 0)) {
        ...
        if ((haki_array_length(arms) > ((int64_t)0LL))) {
            ...
            const char* armTy = inferMatchArmTy(firstArm, sc, sym);
...
const char* inferStmtYieldTy(compiler__Stmt* stmt, Scope* sc, typeck__SymTable* sym) {
...
const char* inferMatchArmTy(compiler__MatchArm* arm, Scope* sc, typeck__SymTable* sym) {
...
TypedExpr* makeTyped(compiler__Expr* e, Scope* sc, typeck__SymTable* sym) {
    const char* ty = inferExprTy(e, sc, sym);
    return ({ TypedExpr* __c_TypedExpr = (TypedExpr*)malloc(sizeof(TypedExpr)); __c_TypedExpr->kind = e; __c_TypedExpr->ty = ty; __c_TypedExpr; });
}

void populateScopeFromParams(Scope* sc, void* params) {
    { void* __arr_p = params;
        ...
        for (int64_t __i_p = 0; __i_p < __len_p; __i_p++) {
            ...
            const char* ty = simplifyTy(p->ty);
            scopeSet(sc, p->name, ty);
        }
    }
}

const char* simplifyTy(const char* ty) {
    if ((strcmp(ty, "int") == 0)) {
        return "int";
    }
    if ((strcmp(ty, "float") == 0)) {
...
void inferFnScope(void* stmts, Scope* sc, typeck__SymTable* sym) {
    { void* __arr_stmt = stmts;
        int64_t __len_stmt = haki_array_length(__arr_stmt);
        for (int64_t __i_stmt = 0; __i_stmt < __len_stmt; __i_stmt++) {
            compiler__Stmt* stmt = *(compiler__Stmt**)haki_array_get(__arr_stmt, __i_stmt);
            ...
            if ((strcmp(tag, "let") == 0)) {
                ...
                const char* ty = inferExprTy(init, sc, sym);
                scopeSet(sc, name, ty);
            }
            if ((strcmp(tag, "if") == 0)) {
...
TypedExpr* inferExpr(compiler__Expr* e, compiler__FnDef* fnDef, typeck__SymTable* sym) {
    Scope* sc = scopeNew();
...
Scope* buildFnScope(compiler__FnDef* fnDef, typeck__SymTable* sym) {
    Scope* sc = scopeNew();
    populateScopeFromParams(sc, fnDef->params);
    inferFnScope(fnDef->body, sc, sym);
    return sc;
...
const char* inferWithScope(compiler__Expr* e, Scope* sc, typeck__SymTable* sym) {
    return inferExprTy(e, sc, sym);
}
